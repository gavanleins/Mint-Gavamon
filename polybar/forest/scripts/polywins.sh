...
generate_window_list() {
    ...
    # Match common apps to icons
		case "$cls" in
			"discord")          icon="ﭮ" ;;
			"firefox"|"librewolf"|"librewolf-default") icon="" ;;
			"Spotify")          icon="" ;;
			"Code")             icon="" ;;
			"steam")            icon="" ;;
			"vlc")              icon="嗢" ;;
			...
			"openai"|"Chat-gpt"|"ChatGPT")  icon="" ;;
			"Thunderbird"|"evolution"|"geary"|"BlueMail") icon="" ;;  # Mail clients
			*)                  icon="$cls" ;;
		esac

		# Use user-selected character case
		case "$char_case" in
			"lower") icon=$(echo "$icon" | tr '[:upper:]' '[:lower:]') ;;
			"upper") icon=$(echo "$icon" | tr '[:lower:]' '[:upper:]') ;;
		esac

		# Truncate and space icon
		if [ "${#icon}" -gt "$char_limit" ]; then
			icon="$(echo "$icon" | cut -c1-$((char_limit-1)))…"
		fi
		...
		if echo "$forbidden_classes" | grep -qw "$cls"; then
			continue
		fi
		...
		if [ "$wid" = "$active_wid" ]; then
			w_name="${active_left}${icon}${active_right}"
		else
			w_name="${inactive_left}${icon}${inactive_right}"
		fi
